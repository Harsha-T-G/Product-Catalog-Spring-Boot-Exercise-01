...
description: Writes PR bodies for Product Catalog using the repo template. Self-contained vendored skill.
---

# write-pr-description

## Product Catalog overrides (read first)

This copy is **vendored in the repository** — do not fetch external skill URLs.

Fill **`.github/pull_request_template.md`** — keep headings and order exactly as written.

| Section | Content |
| --- | --- |
| Description | What + why; `Fixes #` or N/A |
| Why are these changes needed? | Context and rejected alternatives |
| Key Changes | Bullets; REQ/AC/TASK refs when relevant |
| How to Test | `./mvnw clean verify`; optional `./scripts/run-dev.sh` |
| Media | No UI — link `docs/diagrams/` or `docs/curl-commands.sh` |

Pair with `.agents/skills/create-pr/SKILL.md` to open the PR with `gh`.
